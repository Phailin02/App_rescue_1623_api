...
const imageDir = path.join(__dirname, "../../../public/images/unit/");
module.exports = {
  View: async (req, res) => {
    let resp;
    ...
    const baseUrl = process.env.BASE_URL_IMG + "unit/";

    try {
      resp = await ExeQuery(`CALL unit_view(?,?,?)`, [
        search || "",
        start,
        limit,
      ]);
  ...
  Create: async (req, res) => {
    ...
    try {
      resp = await ExeQuery(`CALL unit_insert(?,?,?,?,?,?,?,?,?)`, [
        ...Object.values(body),
        filename,
        req.userModel?.id,
      ]);
  ...
  Delete: async (req, res) => {
    ...
    try {
      resp = await ExeQuery(`CALL unit_delete(?,?)`, [
        body.id,
        req.userModel?.id,
      ]);
  ...
  Update: async (req, res) => {
    ...
    try {
      resp = await ExeQuery(`CALL unit_update(?,?,?,?,?,?,?,?,?,?)`, [
        ...Object.values(body),
        newfilename,
        req.userModel?.id,
      ]);
