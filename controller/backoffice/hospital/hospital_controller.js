...
const imageDir = path.join(__dirname, "../../../public/images/hospital/");
module.exports = {
  View: async (req, res) => {
    let resp;
    ...
    const baseUrl = process.env.BASE_URL_IMG + "hospital/";
    try {
      resp = await ExeQuery(`CALL hospital_view(?,?,?)`, [
        search || "",
        start,
        limit,
      ]);
  ...
  Create: async (req, res) => {
    ...
    try {
      resp = await ExeQuery(`CALL hospital_insert(?,?,?,?,?,?,?,?)`, [
        ...Object.values(body),
        filename,
        req.userModel?.id,
      ]);
  ...
  Delete: async (req, res) => {
    ...
    try {
      resp = await ExeQuery(`CALL hospital_delete(?,?)`, [
        body.id,
        req.userModel?.id,
      ]);
  ...
  Update: async (req, res) => {
    ...
    try {
      resp = await ExeQuery(`CALL hospital_update(?,?,?,?,?,?,?,?,?)`, [
        ...Object.values(body),
        newfilename,
        req.userModel?.id,
      ]);
